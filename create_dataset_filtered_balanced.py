...
def filter_and_split_dataset(
    ...
    """
    Like filter_and_split_dataset in create_dataset_filtered.py, but instead of
    using a fixed per-category cap (n), all available (deduplicated) images are
    gathered and run through the model filter.  After filtering, the category with
    the fewest surviving images determines the dataset size; every other category
    is randomly down-sampled to that count, producing the largest possible
    balanced dataset.
    ...
    # --- Pass 1: gather all candidates, run model filter, store per-category results ---
    kept_paths_per_category = {}

    for category in categories:
        true_label = class_to_idx[category]
        print(f"\nProcessing category: {category}")

        # --- Gather ALL candidates (no n-cap) ---
        candidate_paths = []
        ...
        kept_paths_per_category[category] = kept_paths

    # --- Pass 2: balance to smallest category, then split and copy ---
    min_count = min(len(paths) for paths in kept_paths_per_category.values())
    print(
        f"\nSmallest post-filter category has {min_count} images — using this as the cap for all categories."
    )

    for category in categories:
        kept_paths = kept_paths_per_category[category]
        random.shuffle(kept_paths)
        balanced_paths = kept_paths[:min_count]

        total_images = len(balanced_paths)
        train_end = int(total_images * train_ratio)
        val_end = train_end + int(total_images * val_ratio)

        train_files = balanced_paths[:train_end]
        val_files = balanced_paths[train_end:val_end]
        test_files = balanced_paths[val_end:]

        print(
            f"\n  [{category}] Splitting {total_images} images into:"
            f" {len(train_files)} train,"
            f" {len(val_files)} validate, {len(test_files)} test."
        )

        def copy_files(files, split_name, cat=category):
            for source_path in files:
                filename = os.path.basename(source_path)
                dest_path = dest_dir / split_name / cat / filename
...
final_dataset_folder = f"{base_path}/data/single_label_category_balanced_filtered_auto/"
