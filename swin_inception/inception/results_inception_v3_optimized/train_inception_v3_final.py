"""
Inception V3 FINAL training using best Optuna hyperparameters.
Best Optuna F1: 90.42% — this script runs full 100 epochs to push to 95%+
"""
...
# ── Config with BEST OPTUNA PARAMS ───────────────────────────────────────────
class Config:
    ...
    PATIENCE        = 25
    MIN_DELTA       = 0.0005
    OUTPUT_DIR      = './results_inception_final_v2'
    DEVICE          = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # ── BEST OPTUNA PARAMS (2nd run Trial 47) ─────────────────────────────────
    LR              = 0.0010915660278912984
    WEIGHT_DECAY    = 0.06332487211326991
    DROPOUT         = 0.5992062925899448
    FOCAL_GAMMA     = 1.4905167440097435
    LABEL_SMOOTHING = 0.09414428021554277
    BACKBONE_LR_RATIO = 0.2159851883104212
    MIXUP_ALPHA     = 0.2620760362818748
    CUTMIX_PROB     = 0.23444949372602195
    HIDDEN_SIZE     = 1024
...
def get_class_weights(dataset):
    ...
    total = len(targets)
    weights = {c: total / class_counts[c] for c in class_counts}
    max_w = max(weights.values())
    weights = {c: v / max_w for c, v in weights.items()}
    blurry_idx = dataset.classes.index('Blurry')
    weights[blurry_idx] *= 2.0   # Stronger boost for Blurry
    return weights
...
def get_loaders(config):
    ...
    print(f"Class weights: {[round(cw[i],3) for i in range(config.NUM_CLASSES)]}")
...
def build_model(config):
    ...
    model.fc = nn.Sequential(
        nn.Linear(nf, h),
        nn.BatchNorm1d(h), nn.ReLU(), nn.Dropout(config.DROPOUT),
        nn.Linear(h, h//2),
        nn.BatchNorm1d(h//2), nn.ReLU(), nn.Dropout(config.DROPOUT * 0.8),
        nn.Linear(h//2, config.NUM_CLASSES)
    )
    if hasattr(model, 'AuxLogits'):
        model.AuxLogits.fc = nn.Linear(model.AuxLogits.fc.in_features, config.NUM_CLASSES)
...
def unfreeze(model, phase):
    if phase == 1:
        ...
        for n, p in model.named_parameters():
            if any(x in n for x in ['Mixed_5', 'Mixed_6', 'Mixed_7', 'fc', 'AuxLogits']):
                p.requires_grad = True
        print("Phase 2: Mixed_5+6+7 + classifier")
    elif phase == 3:
...
def make_optimizer(model, config, phase):
    cls_params  = [p for n,p in model.named_parameters() if p.requires_grad and ('fc' in n or 'AuxLogits' in n)]
    back_params = [p for n,p in model.named_parameters() if p.requires_grad and 'fc' not in n and 'AuxLogits' not in n]
    if phase == 1:
        return optim.AdamW(cls_params, lr=config.LR, weight_decay=config.WEIGHT_DECAY)
    else:
        return optim.AdamW([
            {'params': cls_params,  'lr': config.LR},
            {'params': back_params, 'lr': config.LR * config.BACKBONE_LR_RATIO},
...
def train_epoch(model, loader, criterion, optimizer, config, use_aug=True):
    ...
    for x, y in loader:
        ...
        if mixed:
            ...
            if isinstance(out, tuple): loss += 0.4*criterion(out[1], y)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        optimizer.step()
        loss_sum += loss.item()*x.size(0)
        preds_all.extend(torch.argmax(main,1).cpu().numpy())
...
def evaluate(model, loader, criterion, config, tta=False):
    model.eval()
    loss_sum, preds_all, labels_all, probs_all = 0, [], [], []
    with torch.no_grad():
        for x, y in loader:
            x, y = x.to(config.DEVICE), y.to(config.DEVICE)
            if tta:
...
def train_model(config):
    ...
    best_f1, no_improve, global_ep = 0.0, 0, 0
    ...
    for phase, n_epochs, use_aug, use_tta in [
        ...
        for ep in range(n_epochs):
            ...
            print(f"Ep {global_ep+1:3d} P{phase} | Train F1={tr_f1:.4f} Loss={tr_loss:.4f} | "
                  f"Val F1={val_f1:.4f}{tta_tag} | "
                  f"Blurry F1={vm['f1_Blurry']:.4f} R={vm['recall_Blurry']:.4f}")
            if val_f1 > best_f1 + config.MIN_DELTA:
                best_f1 = val_f1
                no_improve = 0
                torch.save({'model_state_dict': model.state_dict(),
                            'val_metrics': vm, 'epoch': global_ep},
    ...
    print(f"\n{'='*60}\nFINAL RESULTS\n{'='*60}")
    print(f"Accuracy:  {fm['accuracy']:.4f} ({fm['accuracy']*100:.2f}%)")
    print(f"F1 Macro:  {fm['f1_macro']:.4f} ({fm['f1_macro']*100:.2f}%)")
