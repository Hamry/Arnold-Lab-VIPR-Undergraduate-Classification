"""
Optuna hyperparameter search for Swin Transformer VIPR classification.
"""
...
MAX_EPOCHS  = 60
N_TRIALS    = 50
OUTPUT_DIR  = './results_swin_optuna'
...
def get_loaders():
    ...
    wt       = torch.FloatTensor([class_w[i] for i in range(NUM_CLASSES)]).to(DEVICE)

    return train_loader, val_loader, wt
...
def build_model(dropout, hidden_size):
    ...
    model.head = nn.Sequential(
        ...
        nn.LayerNorm(hidden_size // 2), nn.GELU(), nn.Dropout(dropout * 0.75),
        nn.Linear(hidden_size // 2, NUM_CLASSES)
    )
    # Unfreeze last Swin stage + classifier
    for name, p in model.named_parameters():
        if 'features.7' in name or 'head' in name:
...
def run_trial(trial, train_loader, val_loader, weight_tensor, params):
    ...
    classifier_params = [p for n,p in model.named_parameters() if p.requires_grad and 'head' in n]
    backbone_params   = [p for n,p in model.named_parameters() if p.requires_grad and 'head' not in n]
    optimizer = optim.AdamW([
        {'params': classifier_params, 'lr': params['lr']},
        {'params': backbone_params,   'lr': params['lr'] * params['backbone_lr_ratio']},
    ], weight_decay=params['weight_decay'])

    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=MAX_EPOCHS, eta_min=1e-8)
    best_f1   = 0.0

    for epoch in range(MAX_EPOCHS):
        if epoch == 20:
            for name, p in model.named_parameters():
                if any(x in name for x in ['features.5', 'features.6', 'features.7', 'head', 'norm']):
                    p.requires_grad = True
        ...
        for inputs, labels in train_loader:
            ...
            optimizer.zero_grad()
            out = model(inputs)
            loss = (lam * criterion(out, ya) + (1-lam) * criterion(out, yb)) if mixed else criterion(out, labels)
            loss.backward()
        ...
        with torch.no_grad():
            for inputs, labels in val_loader:
                preds = torch.argmax(model(inputs.to(DEVICE)), 1).cpu().numpy()
...
def objective(trial):
    params = {
        ...
        'backbone_lr_ratio':trial.suggest_float('backbone_lr_ratio',0.01, 0.2),
        'mixup_alpha':      trial.suggest_float('mixup_alpha',      0.1,  0.5),
        'cutmix_prob':      trial.suggest_float('cutmix_prob',      0.2,  0.7),
...
if __name__ == '__main__':
    study = optuna.create_study(
        ...
        study_name='swin_vipr',
        storage=f'sqlite:///{OUTPUT_DIR}/swin_optuna.db',
        load_if_exists=True
    )
    ...
    print(f'\nSaved to {OUTPUT_DIR}/optuna_best_params.json')