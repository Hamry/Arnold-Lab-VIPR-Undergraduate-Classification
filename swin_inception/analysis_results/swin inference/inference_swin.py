"""
Swin Transformer Inference Script — VIPR / MycorrhiSEE Dataset

Usage:
    python inference_swin.py \
        --model results_swin_final/best_model.pth \
        --data /work/omlpa/SAM \
        --output predictions_swin.csv
"""
...
def infer_hidden_size(state: dict) -> int:
    """
    Read hidden_size directly from the saved head weights so we never
    have to guess or pass a config file.
    head.0.weight shape = (hidden_size, backbone_out_features)
    """
    h = state["head.0.weight"].shape[0]
    print(f"Auto-detected hidden_size={h} from checkpoint weights")
    return h


def build_swin(hidden_size: int, dropout: float = 0.5) -> nn.Module:
    model = models.swin_t(weights=None)
    nf = model.head.in_features
    h  = hidden_size
    model.head = nn.Sequential(
        nn.Linear(nf, h),
        nn.LayerNorm(h), nn.GELU(), nn.Dropout(dropout),
        nn.Linear(h, h // 2),
        nn.LayerNorm(h // 2), nn.GELU(), nn.Dropout(dropout * 0.75),
...
def load_model(pth_path: str) -> nn.Module:
    ...
    model = build_swin(hidden_size=hidden_size)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Swin Transformer inference on PNG directory"
    )
    p.add_argument("--model",      required=True,
                   help="Path to best_model.pth")
    p.add_argument("--data",       required=True,
                   help="Root directory to scan for .png files (recursive)")
    p.add_argument("--output",     default="predictions_swin.csv",
